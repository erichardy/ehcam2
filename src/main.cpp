...
static esp_err_t web_handler(httpd_req_t *req) {
  ...
  char pageWeb[175] = "";
  strcat(pageWeb, "<!doctype html> <html> <head> <title id='title'>ehcam2</title> </head> <body> <img id='stream' src='http://");
  ...
  sprintf (adresse, "%d.%d.%d.%d:%d", WiFi.softAPIP()[0], WiFi.softAPIP()[1], WiFi.softAPIP()[2], WiFi.softAPIP()[3],numero_port);
...
void setup() {
  ...
  Serial.println("WIFI Access point creation... Please wait...");
  WiFi.softAP(ssid);
